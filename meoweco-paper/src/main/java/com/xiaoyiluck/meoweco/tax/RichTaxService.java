...
public class RichTaxService {
    ...
    private void executeCycle(RichTaxSettings settings) {
        try {
            if (settings.destinationType() == DestinationType.PLAYER && settings.destinationPlayerUuid() == null) {
            ...
            RichTaxEngine.Settings engineSettings = new RichTaxEngine.Settings(
                    rules,
                    settings.destinationType() == DestinationType.PLAYER ? RichTaxEngine.DestinationType.PLAYER : RichTaxEngine.DestinationType.SYSTEM,
            ...
            for (RichTaxEngine.CurrencyCycleResult currencyResult : result.perCurrency().values()) {
                ...
                String amount = currency != null
                        ? plugin.formatFixed(currencyResult.collectedAmount(), currency)
                        : String.valueOf(currencyResult.collectedAmount());
                plugin.getLogger().info("Rich tax collected " + amount
                        + " from " + currencyResult.taxedAccounts() + " account(s) in currency '" + currencyResult.currencyId() + "'"
                        + " (threshold=" + currencyResult.threshold() + ", rate=" + currencyResult.rate() + ").");
            }
            ...
            if (!result.hasTaxedAccounts()) {
                ...
                plugin.getLogger().info("Rich tax cycle finished. Taxed accounts=" + result.totalTaxedAccounts() + ", total collected=" + result.totalCollected() + ".");
            }
